...
function openCodeOrchestrateContract(forgeHome) {
  ...
  return [
    ...
    "Treat queued as deferred by the concurrency cap and use task-result events to dispatch the next ready issue. A GitHub issue-search URL is a standing query: after every ordinary task completion, re-run the same raw preflight input and admit only newly matched issues through the refreshed eligibility and dependency plan before declaring the batch complete. Keep the full shared workflow's labels, annotations, leases, and terminal-state rules.",
  ].join("\n");
}
