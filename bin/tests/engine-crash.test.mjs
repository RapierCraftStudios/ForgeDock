...
function makeWorld() {
  ...
  const runner = async ({ commandName }) => {
    switch (commandName) {
      ...
      case "work-on/build": w.markers += " FORGE:BUILDER:COMPLETE"; w.commitsAhead = 2; break;
      case "work-on/review": w.reviewRuns++; w.pr = 7; w.prMerged = true; break;
...
async function runToCompletion({ dir, io, runner }) {
  for (let i = 1; i <= 20; i++) {
    ...
    } catch { /* simulated process death; the durable run-log lets the next launch resume */ }
  }
