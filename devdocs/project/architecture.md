...
Note the direction of concern here. **Boundary Rules** above guard the *outbound* direction — keeping this repo's AGPL code from copyleft-contaminating the proprietary Platform. This section guards the *inbound* direction — whether an external tool's license could copyleft-contaminate **this** AGPL repo. The two are distinct, and the same boundary-isolation reasoning resolves both.
