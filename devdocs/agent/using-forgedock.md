...
**When dogfooding**: `/work-on` reads its own spec from `~/.claude/commands/work-on.md` (symlinked from `commands/work-on.md`). Changes to `commands/work-on.md` take effect in the NEXT session, not the current one.
