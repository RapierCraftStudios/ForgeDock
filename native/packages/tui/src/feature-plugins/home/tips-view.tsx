...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/work-on 42{/highlight} to drive a GitHub issue through the ForgeDock workflow",
  ...
  "Create {highlight}forgedock.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/forgedock/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.forgedock-cli/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.forgedock-cli/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.forgedock-cli/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.forgedock-cli/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent ForgeDock from reading sensitive files",
  "Use {highlight}forgedock-cli run{/highlight} for non-interactive scripting",
  "Use {highlight}forgedock-cli --continue{/highlight} to resume the last session",
  "Use {highlight}forgedock-cli run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}forgedock-cli serve{/highlight} for headless API access to ForgeDock",
  "Use {highlight}forgedock-cli run --attach{/highlight} to connect to a running server",
  "Run {highlight}/orchestrate{/highlight} to execute dependency-aware issue waves",
  "Run {highlight}forgedock-cli auth list{/highlight} to see all configured providers",
  "Run {highlight}forgedock-cli agent create{/highlight} for guided agent creation",
  "Use {highlight}/work-on{/highlight} to execute the full ForgeDock issue lifecycle",
  "Use {highlight}/review-pr{/highlight} for independent specialist review",
  "Use {highlight}/pipeline-status{/highlight} to inspect active GitHub-backed workflow state",
  "Use {highlight}/diagnose{/highlight} to investigate a stalled or failed workflow",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.forgedock-cli/themes/{/highlight} directory",
  ...
  "Run {highlight}forgedock-cli debug config{/highlight} to troubleshoot configuration",
  ...
  "Keep {highlight}forge.yaml{/highlight} committed so workflow policy is shared with the team",
  "Use {highlight}/connect{/highlight} to configure a supported model provider",
