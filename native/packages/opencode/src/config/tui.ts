...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old forgedock.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.forgedock-cli`
  ...
  // 4. `.forgedock-cli` directories (and FORGEDOCK_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".forgedock-cli") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".forgedock-cli") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
