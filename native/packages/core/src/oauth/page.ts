...
// etc.).
//
// The visual language follows ForgeDock's Chrome-and-Ember identity.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `ForgeDock is now connected to ${escapeHtml(provider)}.` : "ForgeDock is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `ForgeDock couldn't finish connecting to ${escapeHtml(provider)}.`
        : "ForgeDock couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from ForgeDock.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · ForgeDock</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("ForgeDock couldn't finish connecting to "+PROVIDER+"."):"ForgeDock couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from ForgeDock."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("ForgeDock is now connected to "+PROVIDER+"."):"ForgeDock is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 48" aria-label="ForgeDock" role="img">
        <defs><linearGradient id="forge-gradient" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#ff4d00"/><stop offset="1" stop-color="#ffd166"/></linearGradient></defs>
        <text x="160" y="34" text-anchor="middle" fill="url(#forge-gradient)" font-family="ui-monospace, SFMono-Regular, Menlo, Consolas, monospace" font-size="32" font-weight="800" letter-spacing="2">FORGEDOCK</text>
      </svg>`
