...
# Same validation convention as BUDGET_LIMIT in Step 4A-pre.0 above.
...
### Step 4A.pre.0: Pre-create milestone branches for ready issues (MANDATORY before classify-lane)
...
**Requires bash 4+**: This snippet uses an associative array (`declare -A SEEN_MILESTONE_SLUG`) to de-dupe milestone slugs, so it must run under bash 4 or newer. Under a non-bash POSIX shell (`sh`/dash), `declare -A` fails and the de-dupe silently no-ops. This degrades gracefully — branch creation stays correct because the `git ls-remote --exit-code` exists-check below still skips any milestone branch that already exists; the only effect is redundant, idempotent `ls-remote`/`push` attempts for milestones referenced by more than one issue. Run this command's blocks under bash 4+.
...
# Same-file current-state brief forwarding. Populated by the core streaming
...
# Per-batch token budget for Step 4C's review-finding cascade control.
...
# Surface-area batching accumulators, promoted to batch scope here so the
# Step 6B report sees the full-run total instead of only the last Step 4C
...
Use `${ISSUE_LANE[$NUM]}` and `${ISSUE_PR_BASE[$NUM]}` to populate `{LANE}` and `{PR_BASE}` in the agent template below. Never substitute prose guesses for these values — the script output is the only valid source.
...
if [ "$FORGEDOCK_AVAILABLE" = "true" ]; then
  ...
  # Concurrency gate: dispatch in chunks of MAX_CONCURRENT, waiting for each
...
**`{GIST_CONTEXT}` generation**: For each issue being dispatched, build the context block. **Prefer the deconflicted `FORGE:SYNTHESIS_BRIEF` (from Phase 2.5) when one exists** — it is a per-issue, already-reconciled brief that carries only the arbitration decisions and sibling investigation Gists relevant to *this* issue. Injecting it instead of the full aggregated milestone-index gist means the agent does not re-arbitrate the same contradictions (less token spend, less nondeterminism). Only when Phase 2.5 did not run (0/1 investigations — no brief exists) does this fall back to the raw parent-investigation + milestone-index gist behavior.
...
**Claims board context injection**: When a coordination issue exists for this batch (`FORGE_COORD_ISSUE` is set), append the claims board URL and the active-claims check instruction to the agent's context. This enables each `/work-on` agent to post its `FORGE:CLAIM` on build start.
...
**Hot-copy CONTRACT context** (extends `{GIST_CONTEXT}` for milestone-lane issues where the parent issue already carries a `FORGE:CONTRACT` annotation):
...
`AGENT_ISSUE_MAP` starts empty and accumulates entries as agents are spawned. For parallel dispatch (all Agent() calls in one message), capture the returned IDs from the batch response — one entry per issue — before entering Step 4B's monitoring loop. Without this capture, `resume=` calls in Steps 4B and 4B.5 will have no agent ID to reference and the resume will fail.

**Launch only up to the concurrency cap** — never put more than `dispatch_headroom` (`MAX_CONCURRENT - ACTIVE_DISPATCH_COUNT`, from Step 4A-pre.0.2) Agent tool calls into a single message. Use `run_in_background=true` so the dispatched agents execute in parallel with each other, within the cap.

```bash
# Compute this dispatch batch: merge any previously-deferred concurrency
...
for BLOCKED_NUM in {all_blocked_issue_numbers}; do
  ...
  for PRED in {predecessors_of_BLOCKED_NUM}; do
    ...
    case "$PRED_STATE" in
      DONE)
        # Same-file current-state brief forwarding: only fires when the
...
4. **Post CLAIM_RELEASED on coordination issue** (when `FORGE_COORD_ISSUE` is set):
   ```bash
...
**Released**: $(date -u +%Y-%m-%dT%H:%M:%SZ)" 2>/dev/null || true
   ...
   **Claims-board relaxation sweep** (run after posting CLAIM_RELEASED): When a claim is released, check all remaining Layer-2/4-serialized issue pairs. If the now-released Holder's claimed files were the *only* conflict reason for a still-blocked issue, and that blocked issue already has an active `FORGE:CLAIM` with a disjoint file set, the blocking edge MAY be relaxed (blocked issue becomes ready).
...
6.5. **Handle predecessor gating** (`GATED` — `needs-human` or `workflow:awaiting-merge`) — if a completed agent's issue classifies as `GATED`, its direct dependents are neither dispatched nor marked failed/skipped. For each direct dependent `DEP` of the gated predecessor `PRED`:

   **Edge re-verification gate (run FIRST, before any tracking)**:
   ```bash
   ...
   **Existing tracking logic (unchanged for the KEEP case)**:
   ...
   # a bare contains("#N") substring would false-match #50/#500 for predecessor #5.
...
6.7. **Human-gated idle/backpressure check** (`BATCH_FULLY_GATED`) — run this after every completion cycle, once the per-issue classification above (items 5-6.6) has been applied for this cycle. It answers a different question than the paused-drain/blocked-on-human-merge tracking above: those items handle *individual* gated predecessors and their *direct* dependents; this check asks whether the **entire original batch** has now exhausted into human-gated states, which is the condition under which continuing to dispatch cascade-spawned review findings (Step 4C) produces net-negative churn — closing 1 issue while opening 2-4 more, with the real blockers (the GATED issues) unresolved:
...
for NUM in {active_issue_numbers}; do
  ...
  # workflow:awaiting-merge MUST be included here — otherwise the stall detector
...
for FINDING_NUM in {spawned_finding_numbers}; do
  ...
  # keep all sites byte-identical (forge#1837: mirrored checks must not drift).
  ...
  # item 6.7; read it here, do not recompute.
...
**Surface-area batching for queued P3 findings (MANDATORY check before dispatch):**
...
# classify the same issue body differently.
...
# so the loop can never scale API calls linearly with cascade-seeded findings.
MAX_BATCH_SCAN=50
SCANNED=0
...
for FINDING_NUM in "${QUEUED_FINDINGS[@]}"; do
  ...
  # Schema-tolerant: matches canonical priority:P3 or bare P3.
...
# them. This is what makes SURFACE_BATCHED_FINDINGS a live control, not dead wiring.
for FILE in "${!SURFACE_FILE_MEMBERS[@]}"; do
  ...
  # boundary from an untrusted issue body. Shared guard with phase-1-resolve.md.
...
**Per-batch token budget gate (P3-and-below only — MANDATORY, runs on the POST-clubbing `QUEUED_FINDINGS` list):**
...
for FINDING_NUM in "${QUEUED_FINDINGS[@]}"; do
  # Schema-tolerant extraction — same pattern as the Step 4C main loop above.
...
### Step 4C.5: Milestone lane-consistency check (periodic)
...
- **Dependency cascade**: Re-run `classify_predecessor_state` (Step 4B) for the failed/gated issue before cascading — do NOT assume every entry above is a hard failure:
...
### Step 4F: Completion Sweep (deferred review-spawned findings)
...
for FINDING_NUM in "${DEFERRED_FINDINGS[@]}"; do
  ...
  # "Batch fully human-gated" deferrals are their OWN bucket — they must NOT be
  ...
  # "Token budget exceeded" deferrals are ALSO their own bucket — re-evaluating
...
for FINDING_NUM in "${SWEEP_CANDIDATES[@]}"; do
  ...
  # Schema-tolerant extraction, mirrors the Step 4C main loop above.
...
**Step 4F.2.5: Re-evaluate idle-gated deferrals**
...
**Step 4F.2.6: Re-evaluate token-gated deferrals**
...
- Overriding generation >= 2 deferrals **inside this Step 4C mid-run triage** — the cascade cap is absolute for autonomous cascade: a sweep or triage pass must never promote a gen≥2 finding to executed just because it looks low-risk. This is distinct from an explicit human request made *before* the run starts: an operator may deliberately admit gen≥2 findings via `phase-1-resolve.md`'s `cascade`/`review-findings`/`findings` resolution with `--include-deferred`/`--allow-gen2` (or the `orchestration.cascade.max_generation` config lever from #2234) — that is the sanctioned override path, entered at Phase 1 resolution, not a violation of this rule. Do not use that human-request path as precedent to relax Step 4C itself.
- Skipping the sweep because "there are only a few" deferred findings — even one deferred finding represents unresolved work.
- Clearing an idle-gated deferral without recomputing `BATCH_FULLY_GATED` fresh at sweep time — a stale "not gated" read would re-introduce the exact net-negative churn this policy exists to stop.
- Clearing a token-gated deferral by reusing the drained `BATCH_TOKEN_SPEND` instead of the fresh `SWEEP_TOKEN_SPEND` allowance — the whole point of Step 4F.2.6 is that the *original* batch's spend is no longer relevant once it has drained.

### Step 4F.5: Budget Deferred-Issues Report (conditional)
