...
If `SYNTHESIS_RAN` is false, do NOT execute Steps 2.5B–2.5D — proceed directly to Phase 3. Step 4A's `{DISPATCH_CONTEXT}` generation will use the repository-scoped investigator-comment behavior (no synthesis brief exists).
...
for INV_NUM in {investigation_numbers}; do
  ...
  INV_SUBSYSTEM[$INV_NUM]=$(printf '%s' "$INV_BODY" \
...
for ISSUE_NUM in {implementation_issue_numbers}; do
  ...
  # its reserved markers remain in the authoritative source comment.
...
The `FORGE:SYNTHESIS_BRIEF` annotation is consumed by Step 4A's `{DISPATCH_CONTEXT}` generation (which prefers the reconciled repository brief when present) and its reconciled count (`RECONCILED_COUNT`) feeds the Step 6B `Competing recommendations reconciled (Phase 2.5)` metric.
