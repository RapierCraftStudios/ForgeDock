...
For every completed Wave 0 investigation, resolve the exact completed `FORGE:INVESTIGATOR` comment resource. The numeric comment identity and repository issue URL are the durable handoff; a bounded recommendation excerpt is included for prompt context without copying reserved annotation bodies into another annotation. Paginate explicitly so an issue with more than 100 comments cannot hide its latest investigator result.

```bash
# Build a map: investigation number → exact completed investigator comment + bounded summary.
# The map is keyed by the complete investigation set, not by optional external artifacts.
...
for INV_NUM in {investigation_numbers}; do
  ...
  if [ -z "$INVESTIGATOR_JSON" ] || [ "$INVESTIGATOR_JSON" = "null" ]; then
    INVESTIGATION_CONTEXT[$INV_NUM]="Investigation #${INV_NUM}: unavailable — no validated completed FORGE:INVESTIGATOR comment resource was found. Do not substitute unbounded external context; report this missing context explicitly."
...
When spawning agents for implementation issues in Step 4A, include the exact repository comment reference and bounded recommendation from the parent or relevant investigation in the `{DISPATCH_CONTEXT}` context block. The numeric comment identity remains authoritative; workers may fetch that single comment during context gathering for full details. No unbounded external-memory lookup is part of this handoff.
