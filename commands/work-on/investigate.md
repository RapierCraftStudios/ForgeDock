...
## Phase 0.5: Repository Context Retrieval — Prior Run Priors <!-- Added: forge#1316 -->

**Goal**: Establish the repository-scoped context path before investigation. There is no separate external memory retrieval in this phase; the following Phase 0.6 performs the single Forge Ledger pre-recall using issue terms, affected paths, and symbols.

**This phase is non-blocking** — Phase 0.6 proceeds from an absent/empty Ledger without stalling the pipeline. Do not scan unrelated repositories or fetch external artifacts here.

Historical Gists are preserved as immutable legacy artifacts. This pipeline does not delete, rewrite, fetch, or migrate them. Any separately authorized migration must preserve a verified read-back copy and record inaccessible legacy references before any authorized transfer; this issue does not authorize migration.
...
**Relationship to Phase 0.5**: Phase 0.5 provides the prior-run framing and Phase 0.6 performs the repository-scoped Forge Ledger query (BM25 over structured knowledge cards, indexed by `build-knowledge-index.mjs`). They are complementary views over the same durable repository-local knowledge surface.
...
## Phase 1C.5 and 1C.6: Repository-Scoped Handoff

The investigator comment is the durable cross-phase artifact. No external memory artifact is created, indexed, or updated here. Child propagation uses the exact parent issue/comment reference emitted by `work-on/decompose.md`, and build context validates that reference before consuming it.

The `FORGE:INVESTIGATOR` comment and its `INVESTIGATION:COMPLETE` or `INVESTIGATION:INVALID` sentinel remain unchanged. The Forge Ledger remains the only prior-knowledge lookup and is non-blocking.

Historical Gists are preserved as immutable legacy artifacts. This phase does not delete, rewrite, fetch, or migrate them. Any separately authorized migration must preserve a verified read-back copy and record inaccessible legacy references before any authorized transfer; this issue does not authorize migration.
